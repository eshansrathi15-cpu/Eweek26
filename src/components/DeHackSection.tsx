...
const DeHackSection = () => {
  return <section id="features" className="py-24 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        repeat: Infinity,
        repeatDelay: 5
      }} className="h-px bg-primary absolute" style={{
        top: `${i * 5}%`
      }} />)}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          duration: 0.8
            ...
            <motion.h2 className="text-6xl md:text-8xl font-mono font-bold text-foreground mb-6" whileHover={{
            scale: 1.02
          }}>
              DE<span className="animate-glitch">HACK</span>
            </motion.h2>

            <p className="text-xl md:text-2xl text-muted-foreground mb-8 font-sans"> Build. Break. Innovate.</p>
          ...
          delay: 0.2
            ...
            <div className="aspect-square border-2 border-foreground relative overflow-hidden bg-background">
              ...
              <div className="font-mono text-sm text-muted-foreground space-y-2 overflow-hidden p-6 pt-12">
                {['> initializing hackathon...', '> loading participants: 500+', '> allocating resources...', '> starting timer: 120:00:00', '> BUILD MODE ACTIVATED', '', 'function innovate() {', '  const ideas = [];', '  while (time > 0) {', '    ideas.push(create());', '    iterate();', '    break_things();', '  }', '  return revolution;', '}'].map((line, i) => <motion.p key={i} initial={{
                opacity: 0,
                x: -20
              }} whileInView={{
                opacity: 1,
