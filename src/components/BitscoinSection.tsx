...
const BitscoinSection = () => {
  ...
  return (
    <section className="py-24 relative px-6 overflow-hidden bg-background">
      {/* Cinematic Grain/Vignette Overlay */}
      <div className="absolute inset-0 pointer-events-none vignette opacity-40" />

      <div className="max-w-6xl mx-auto relative z-10">
        {/* Header - Styled like your Hero "Now Showing" badge */}
        <div className="flex flex-col items-center mb-16">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 mb-4 px-4 py-2 border border-primary/50 bg-primary/10"
          >
          ...
          <h2 className="text-6xl md:text-7xl font-mono font-bold text-center tracking-tighter">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {cards.map((card, index) => (
            <motion.div
              ...
              className="relative p-8 border border-muted group hover:border-primary/50 transition-colors duration-500 bg-black/40 backdrop-blur-sm"
            >
              {/* Ticket Perforation effect on cards */}
              <div className="absolute -top-3 -left-3 w-6 h-6 bg-background border border-muted rounded-full" />
              <div className="absolute -bottom-3 -right-3 w-6 h-6 bg-background border border-muted rounded-full" />
        ...
        >
          <div className="inline-block ticket-perf h-1 w-full max-w-md bg-muted mb-6" />
          <p className="font-mono text-xs tracking-[0.3em] text-muted-foreground uppercase">
            A CEL Original Production • All Rights Reserved
          </p>
        </motion.div>
      </div>
    </section>
